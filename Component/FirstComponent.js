...
// 创建一个简单的React组件
import { View, Text, StyleSheet, SafeAreaView, FlatList, TouchableOpacity, Modal, ActivityIndicator, ScrollView } from 'react-native';

export const List = ({ items = [
  '北京', '天津', '上海', '广州', '深圳', '杭州', '成都', '重庆', '天津', '南京', '武汉',
  '西安', '苏州', '青岛', '沈阳', '大连', '郑州', '长沙', '合肥', '福州', '厦门',
  '济南', '哈尔滨', '长春', '石家庄', '昆明', '南昌', '南宁', '贵阳', '太原', '呼和浩特',
  '乌鲁木齐', '兰州', '银川', '西宁', '拉萨', '香港', '澳门', '台北', '高雄', '新竹',
  '莫斯科', '伦敦', '纽约', '巴黎', '东京', '首尔', '新加坡', '悉尼', '洛杉矶', '温哥华'
] }) => {
  ...
  const handlePress = async (idx) => {
    ...
    const url = `https://baike.baidu.com/item/${encodeURIComponent(city + '市')}?fromModule=lemma_search-box`;
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          <TouchableOpacity onPress={() => setModalVisible(false)}>
            <Text style={{ fontSize: 18, color: '#007AFF', padding: 16 }}>关闭</Text>
          </TouchableOpacity>
          <ScrollView style={{ padding: 16 }}>
            <Text style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 10 }}>{currentCity}市</Text>
            {loading ? (
